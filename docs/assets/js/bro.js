/*
 * ==========================================================================================
 *  Bro Documentation JavaScript
 *  File: docs/assets/js/bro.js
 *
 *  Purpose:
 *      Provides safe progressive enhancements for the Bro MkDocs Material site.
...
( function()
{
	...
	const BroDocs = {
		config: {
			scrollTopId: "bro-scroll-top",
			pageToolsId: "bro-page-tools",
			progressId: "bro-reading-progress",
			tableFilterClass: "bro-table-filter",
			headingLinkClass: "bro-heading-link",
			codeLabelClass: "bro-code-label",
			codeToggleClass: "bro-code-toggle",
			initializedAttribute: "data-bro-enhanced",
			navScrollKey: "bro-docs-nav-scroll",
			...
			tableSelector: ".md-typeset table:not([data-bro-no-filter])",
		...
		handleDocumentClick: function( event )
		{
			...
			if( target.closest && target.closest( "[data-bro-copy-heading]" ) )
			{
				event.preventDefault();
				this.copyHeadingLink( target.closest( "[data-bro-copy-heading]" ) );
				return;
			}
			if( target.closest && target.closest( "[data-bro-copy-page]" ) )
			{
				event.preventDefault();
				this.copyPageLink( target.closest( "[data-bro-copy-page]" ) );
				return;
			}
			if( target.closest && target.closest( "[data-bro-print-page]" ) )
			{
				event.preventDefault();
				window.print();
				return;
			}
			if( target.closest && target.closest( "[data-bro-toggle-code]" ) )
			{
				event.preventDefault();
				this.toggleCodeBlock( target.closest( "[data-bro-toggle-code]" ) );
				return;
			}
			if( target.closest && target.closest( "[data-bro-api-expand]" ) )
			{
				event.preventDefault();
				this.setApiDetailsState( true );
				return;
			}
			if( target.closest && target.closest( "[data-bro-api-collapse]" ) )
			{
				event.preventDefault();
				this.setApiDetailsState( false );
				return;
			}
			if( target.closest && target.closest( "[data-bro-api-clear]" ) )
			{
				event.preventDefault();
				this.clearApiFilter();
			}
		...
		handleKeyboardShortcuts: function( event )
		{
			...
			{
				const apiSearch = document.getElementById( "bro-api-search" );
				if( apiSearch )
				{
					event.preventDefault();
					apiSearch.focus();
			...
			links.forEach( function( link )
			{
				try
				{
					...
					if( url.host && url.host !== currentHost )
					{
						...
						link.classList.add( "bro-external-link" );
						if( !link.querySelector( ".bro-external-indicator" ) )
						{
							const indicator = document.createElement( "span" );
							indicator.className = "bro-external-indicator";
		...
		customizeSearch: function()
		{
			...
			{
				input.setAttribute( "placeholder", "Search Bro docs..." );
				input.setAttribute( "aria-label", "Search Bro documentation" );
			} );
		},
		...
		addScrollTopButton: function()
		{
			...
			button.type = "button";
			button.className = "bro-scroll-top";
		...
		addPageTools: function()
		{
			...
			tools.className = "bro-page-tools";
			tools.innerHTML = [
				"<button type=\"button\" data-bro-copy-page title=\"Copy page link\" aria-label=\"Copy page link\">Copy link</button>",
				"<button type=\"button\" data-bro-print-page title=\"Print page\" aria-label=\"Print page\">Print</button>"
			].join( "" );
		...
		{
			const button = document.querySelector( "[data-bro-copy-page]" );
			this.copyTextToClipboard( window.location.href, button, "Copied", "Copy link" );
		},
		addHeadingLinks: function()
		{
			...
			{
				if( heading.querySelector( "." + BroDocs.config.headingLinkClass ) )
				{
					return;
				}
				const button = document.createElement( "button" );
				button.type = "button";
				button.className = BroDocs.config.headingLinkClass;
				button.setAttribute( "data-bro-copy-heading", heading.id );
		...
		{
			const id = button.getAttribute( "data-bro-copy-heading" );
			if( !id )
			{
				return;
			}
		...
		addTableFilters: function()
		{
			...
			{
				if( table.getAttribute( "data-bro-filtered" ) === "true" )
				{
					return;
				}
				...
				if( rows.length < BroDocs.config.largeTableMinimumRows )
				{
					return;
				}
				table.setAttribute( "data-bro-filtered", "true" );
				const wrapper = document.createElement( "div" );
				wrapper.className = "bro-table-tools";
				const input = document.createElement( "input" );
				input.type = "search";
				input.className = BroDocs.config.tableFilterClass;
				input.placeholder = "Filter table...";
				input.setAttribute( "aria-label", "Filter table " + ( index + 1 ) );
				const count = document.createElement( "span" );
				count.className = "bro-table-count";
				...
				{
					BroDocs.filterTable( table, input.value, count );
				} );
			} );
		},
		...
		addCodeLabels: function()
		{
			...
			codeBlocks.forEach( function( code )
			{
				...
				if( !pre || pre.getAttribute( "data-bro-labeled" ) === "true" )
				{
					return;
				}
				const language = BroDocs.detectCodeLanguage( code );
				if( !language )
				{
					return;
				}
				pre.setAttribute( "data-bro-labeled", "true" );
				const label = document.createElement( "div" );
				label.className = BroDocs.config.codeLabelClass;
		...
		addCodeToggles: function()
		{
			...
			{
				const pre = code.parentElement;
				if( !pre || pre.getAttribute( "data-bro-toggle-ready" ) === "true" )
				{
					return;
				}
				pre.setAttribute( "data-bro-toggle-ready", "true" );
				if( pre.scrollHeight <= BroDocs.config.maxCollapsedCodeHeight + 80 )
				{
					return;
				}
				pre.classList.add( "bro-code-collapsed" );
				pre.style.maxHeight = BroDocs.config.maxCollapsedCodeHeight + "px";
				const button = document.createElement( "button" );
				button.type = "button";
				button.className = BroDocs.config.codeToggleClass;
				button.setAttribute( "data-bro-toggle-code", "collapsed" );
		...
		toggleCodeBlock: function( button )
		{
			...
			const state = button.getAttribute( "data-bro-toggle-code" );
			if( state === "collapsed" )
			{
				pre.classList.remove( "bro-code-collapsed" );
				pre.style.maxHeight = "";
				button.setAttribute( "data-bro-toggle-code", "expanded" );
		...
		enhanceKeyboardFocus: function()
		{
			document.body.addEventListener( "keydown", function( event )
			{
				if( event.key === "Tab" )
				{
					document.body.classList.add( "bro-keyboard-mode" );
				}
			} );
			document.body.addEventListener( "mousedown", function()
			{
				document.body.classList.remove( "bro-keyboard-mode" );
			} );
		},
		enhanceApiReference: function()
		{
			...
			{
				if( container.getAttribute( "data-bro-api-enhanced" ) === "true" )
				{
					return;
				}
				container.setAttribute( "data-bro-api-enhanced", "true" );
				const heading = container.querySelector( "h2, h3, h4, h5" );
				if( !heading || heading.querySelector( ".bro-api-badge" ) )
				{
					return;
				}
				const badge = document.createElement( "span" );
				badge.className = "bro-api-badge";
		...
		addApiTools: function()
		{
			...
			if( content.querySelector( ".bro-api-tools" ) )
			{
				return;
			}
			...
			panel.className = "bro-api-tools";
			panel.setAttribute( "aria-label", "API tools" );
			panel.innerHTML = [
				"<h2 class=\"bro-api-tools-title\">API Tools</h2>",
				"<label class=\"bro-api-search-label\" for=\"bro-api-search\">Filter classes, methods, properties, or text</label>",
				"<input id=\"bro-api-search\" class=\"bro-api-search\" type=\"search\" placeholder=\"Filter classes, methods, properties, or text...\" autocomplete=\"off\">",
				"<div class=\"bro-api-tool-buttons\">",
				"<button type=\"button\" class=\"bro-api-tool-button\" data-bro-api-expand>Expand all</button>",
				"<button type=\"button\" class=\"bro-api-tool-button\" data-bro-api-collapse>Collapse all</button>",
				"<button type=\"button\" class=\"bro-api-tool-button\" data-bro-api-clear>Clear filter</button>",
				"</div>",
				"<p class=\"bro-api-filter-status\" aria-live=\"polite\"></p>"
			].join( "" );
			firstHeading.insertAdjacentElement( "afterend", panel );
			const input = panel.querySelector( "#bro-api-search" );
			const status = panel.querySelector( ".bro-api-filter-status" );
			if( input )
			{
				input.addEventListener( "input", function()
				{
					BroDocs.filterApiObjects( input.value, status );
				} );
			}
		},
		filterApiObjects: function( query, statusElement )
		{
			...
			objects.forEach( function( object )
			{
				...
				if( !normalizedQuery || text.indexOf( normalizedQuery ) !== -1 )
				{
					object.classList.remove( "bro-api-hidden" );
					visibleCount += 1;
				}
				else
				{
					object.classList.add( "bro-api-hidden" );
				}
			} );
			if( statusElement )
			{
		...
		{
			const input = document.getElementById( "bro-api-search" );
			const status = document.querySelector( ".bro-api-filter-status" );
			if( !input )
			{
				return;
			}
		...
		enhanceTocProgress: function()
		{
			...
			if( !toc || toc.getAttribute( "data-bro-toc-enhanced" ) === "true" )
			{
				return;
			}
			toc.setAttribute( "data-bro-toc-enhanced", "true" );
			const marker = document.createElement( "div" );
			marker.className = "bro-toc-marker";
		...
		updateTocProgress: function()
		{
			...
			tocLinks.forEach( function( link )
			{
				...
				if( href === activeId )
				{
					link.classList.add( "bro-toc-active" );
				}
				else
				{
					link.classList.remove( "bro-toc-active" );
				}
			} );
		},
		addMermaidGuard: function()
		{
			...
			{
				block.setAttribute( "data-bro-mermaid-detected", "true" );
			} );
		}
	};
